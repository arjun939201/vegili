...
## Security and remaining MVP limits

The project includes database-backed auth rate limiting, same-origin checks for state-changing requests that include an `Origin` header, Alembic migrations, and automated tests. It has not undergone an independent security audit and should still be treated as an MVP. Before a broad public launch, add abuse reporting/moderation, password reset, image uploads, persistent WebSocket fan-out across multiple instances, and operational monitoring/maintenance. Email verification records and rate-limit events are stored in the database and survive process restarts. Expired OTP rows are rejected during verification; periodic cleanup of expired OTP rows can be added as an operational maintenance task. Rate-limit event records older than one day are pruned during auth requests. The limiter uses the request client's peer address; deployments behind proxies should configure trusted proxy handling deliberately and must not blindly trust user-supplied forwarding headers. The Origin check allows requests without an Origin header for non-browser/API clients, so it is not a complete CSRF token system.
